...
/*
 */
String rsaSign(Uint8List inBytes, RSAPrivateKey privateKey) {
  Signer signer = new Signer("SHA-256/RSA");
  signer.init(true, PrivateKeyParameter<RSAPrivateKey>(privateKey));

  Signature signature = signer.generateSignature(inBytes);
...
List<String> _chunked(String encoded, {chunkSize = 64}) {
  ...
  for (num i = 0; i < encoded.length; i += chunkSize) {
    num end = (i + chunkSize < encoded.length) ? i + chunkSize : encoded.length;
    chunks.add(encoded.substring(int.parse(i.toString()), int.parse(end.toString())));
  }
